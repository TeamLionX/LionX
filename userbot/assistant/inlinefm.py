...
from userbot..funcs.decorators import check_owner
from userbot.helpers import humanbytes, progress
from userbot.helpers.utils import _lionutils
...
# BACK
@lionub.tgbot.on(CallbackQuery(pattern="fmback"))
@check_owner
...
# UP
@lionub.tgbot.on(CallbackQuery(pattern="fmup_(.*)"))
@check_owner
async def up(event):
...
# DOWN
@lionub.tgbot.on(CallbackQuery(pattern="fmdown_(.*)"))
@check_owner
...
# FORTH
@lionub.tgbot.on(CallbackQuery(pattern="fmforth_(.*)"))
@check_owner
...
# REMOVE
@lionub.tgbot.on(CallbackQuery(pattern="fmrem_(.*)"))
@check_owner
async def remove(event):
    ...
    await asyncio.sleep(1)
    await event.edit(msg, buttons=buttons)
    await _lionutils.runcmd(f"rm -rf '{rpath}'")
    await event.answer(f"{rpath} removed successfully...")


# SEND
@lionub.tgbot.on(CallbackQuery(pattern="fmsend"))
@check_owner
...
# CUT
@lionub.tgbot.on(CallbackQuery(pattern="fmcut_(.*)"))
@check_owner
async def cut(event):
...
# COPY
@lionub.tgbot.on(CallbackQuery(pattern="fmcopy_(.*)"))
@check_owner
...
# PASTE
@lionub.tgbot.on(CallbackQuery(pattern="fmpaste_(.*)"))
@check_owner
