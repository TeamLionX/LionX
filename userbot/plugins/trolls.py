...
async def lionxbot(event):
    ...
    lionx = await trash(lionx)
    os.remove(download_location)
    await output[0].delete()
    await event.client.send_file(event.chat_id, lionx, reply_to=lionxid)
...
async def lionxbot(event):
    ...
    try:
        ...
        return await output[0].edit(f"**Error: **\n`{exc}`")
    lionx = f"https://telegra.ph{response[0]}"
    lionx = await trap(text1, text2, lionx)
    await output[0].delete()
    os.remove(download_location)
    await event.client.send_file(event.chat_id, lionx, reply_to=lionxid)
...
async def lionxbot(event):
    ...
    try:
        ...
        return await output[0].edit(f"**Error: **\n`{exc}`")
    lionx = f"https://telegra.ph{response[0]}"
    lionx = await phcomment(lionx, text, username)
    await output[0].delete()
    os.remove(download_location)
    await event.client.send_file(event.chat_id, lionx, reply_to=lionxid)
