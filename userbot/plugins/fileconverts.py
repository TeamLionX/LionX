...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    if output is None:
        return await edit_delete(lionevent, "__Unable to make spin gif.__")
...
async def video_lionfile(event):  # sourcery no-metrics
    ...
    flag = True
    lionevent = await edit_or_reply(event, "`Converting to round format..........`")
    lionfile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not lionfile.endswith((".webp")):
            if lionfile.endswith((".tgs")):
                hmm = await make_gif(lionevent, lionfile)
                os.rename(hmm, "./temp/circle.mp4")
                lionfile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(lionfile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = min(height, width)
                await _lionutils.runcmd(
                    f'ffmpeg -i {lionfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(lionfile, PATH)
            if str(lionfile) != str(PATH):
                os.remove(lionfile)
            try:
                lionthumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            lionthumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(lionfile, lionthumb)
        if lionthumb is not None and not os.path.exists(lionthumb):
            lionthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, lionthumb)
        if (
            lionthumb is not None
            and not os.path.exists(lionthumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            lionthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, lionthumb)
        if lionthumb is not None and os.path.exists(lionthumb):
            await _lionutils.runcmd(
                f"""ffmpeg -loop 1 -i {lionthumb} -i {lionfile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(lionfile)
        else:
            os.remove(lionfile)
            return await edit_delete(
                lionevent, "`No thumb found to make it video note`", 5
            )
    if (
        mediatype
        in [
        ...
        and not lionfile.endswith((".webp"))
    ):
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, lionevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=lionid)
    await lionevent.delete()
...
async def on_file_to_photo(event):
    ...
    liont = await edit_or_reply(event, "`Converting.....`")
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    lionevent = await edit_or_reply(event, "__🎞 Making Gif from the relied media...__")
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    lionevent = await edit_or_reply(event, "__🎞Converting into Gif..__")
