...
async def get_users(show):
    ...
    lionxevent = await edit_or_reply(show, "`getting users list wait...`  ")
    try:
        ...
        mentions += f" {str(e)}" + "\n"
    await edit_or_reply(lionxevent, mentions)
...
async def info(event):
    ...
    lionxevent = await edit_or_reply(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event, lionxevent)
    if chat is None:
        return
    caption = await fetch_info(chat, event)
    try:
        await lionxevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
        ...
        await lionxevent.edit("`An unexpected error has occurred.`")


async def get_chatinfo(event, lionxevent):
    ...
    try:
        ...
        try:
            ...
            await lionxevent.edit("`Invalid channel/group`")
            return None
            ...
            await lionxevent.edit("`Channel or supergroup doesn't exist`")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await edit_delete(lionxevent, "**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
