...
async def very(event):  # sourcery no-metrics
    ...
    LOGO_BACKGROUND = (
        ...
        or "https://raw.githubusercontent.com/TeamLionX/Files/main/backgroud/black.jpg"
    )

    LOGO_FONT = (
        gvarstatus("LOGO_FONT")
        or "https://github.com/TeamLionX/Files/blob/main/fonts/Streamster.ttf?raw=true"
    )
...
async def bad(event):
    ...
    for i, each in enumerate(links, start=1):
        lionx = os.path.splitext(each.text)[0]
        bg_name.append(lionx)
        lbg_list += f"**{i}.**  `{lionx}`\n"
    ...
    if cmd == "c":
        ...
        if not input_str and event.reply_to_msg_id and reply_message.media:
            if not os.path.isdir("./temp"):
                os.mkdir("./temp")
            output = await _lionxtools.media_to_pic(event, reply_message)
...
async def pussy(event):  # sourcery no-metrics
    ...
    if cmd == "":
        ...
        if input_str not in logo_font:
            ...
            await asyncio.sleep(1)
            await edit_delete(lionxevent, font_name, time=80)
        else:
            if " " in input_str:
        ...
        if input_str not in fg_name:
            lionxevent = await edit_or_reply(event, "`Give me a correct color name...`")
            await asyncio.sleep(1)
            await edit_delete(
                lionxevent,
                f"**Available color names are here:-**\n\n{fg_list}",
        ...
        lionx = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
        isint = re.match(lionx, input_str)
