...
async def remove_background(event):
    ...
    if event.reply_to_msg_id and not input_str:
        ...
        lionxevent = await edit_or_reply(event, "`Removing Background of this media`")
    ...
    if "image" in contentType:
        ...
        await edit_delete(lionxevent, f"`{response.content.decode('UTF-8')}`", 5)
        return
    if cmd == "srmbg":
