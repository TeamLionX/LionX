...
async def group_fban(event):
    ...
    if errors != []:
        ...
        for txt in errors:
            success_report += f"\n☞ __{txt}__"
    await edit_or_reply(lionevent, success_report)
...
async def quote_search(event):  # sourcery no-metrics
    ...
    if get_collection("fedids") is not None:
        ...
        feds = {}
    if fedgroup == "-all":
        lionevent = await edit_or_reply(event, "`Adding all your feds to database...`")
        fedidstoadd = []
        async with event.client.conversation("@MissRose_bot") as conv:
            try:
                ...
                if "can only" in response.text:
                    return await edit_delete(lionevent, f"__{response.text}__")
                ...
                await edit_delete(
                    lionevent, f"**Error while fecthing myfeds:**\n__{e}__", 10
                )
...
async def fetch_fedinfo(event):
    ...
    lionevent = await edit_or_reply(event, "`Fetching info about given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
            await conv.send_message("/fedinfo " + input_str)
            response = await conv.get_response()
            await lionevent.edit(response.text)
        except YouBlockedUserError:
            await edit_delete(
                lionevent,
            ...
            await edit_delete(
                lionevent, f"**Error while fecthing fedinfo:**\n__{e}__", 10
            )
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
...
async def fetch_fedinfo(event):
    ...
    lionevent = await edit_or_reply(event, "`Fetching admins list of given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
...
async def myfeds_fedinfo(event):
    ...
    lionevent = await edit_or_reply(event, "`Fetching list of feds...`")
...
async def fstat_rose(event):
    ...
    lionevent = await edit_or_reply(event, "`Fetching fedstat from given deatils...`")
    user, fedid = await get_user_from_event(
        event, lionevent, secondgroup=True, noedits=True
    )
    if user is None:
        user = await event.client.get_me()
    if fedid is None:
    ...
    async with event.client.conversation(rose) as conv:
        try:
            ...
            if "Looks like" in response.message:
                ...
                await lionevent.delete()
                return
            await lionevent.edit(result + response.text)
        except YouBlockedUserError:
            await edit_delete(
                lionevent,
            ...
            await edit_delete(
                lionevent, f"**Error while fecthing fedstat:**\n__{e}__", 10
            )
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
