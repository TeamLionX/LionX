...
async def _(event):
    ...
    user, rank = await get_user_from_event(event, lionevent)
    if not rank:
        rank = "Admin"
    if not user:
        return
    try:
        ...
        return await lionevent.edit("__I think you don't have permission to promote__")
    except Exception as e:
        return await edit_delete(lionevent, f"__{e}__", time=10)
