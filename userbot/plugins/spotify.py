...
#
# tg- @TeamLionX  // Github.com/TeamLionX
...
from . import BOTLOG, BOTLOG_CHATID, Config, lionxub, reply_id
        ...
        while SP_DATABASE.SPOTIFY_MODE:
            ...
            if r.status_code == 200:
                ...
                if received["currently_playing_type"] == "track":
                    ...
                    if save_spam("spotify", False):
                        ...
                        await lionxub.send_message(BOTLOG_CHATID, string)
                else:
            ...
            # need to change anything
                ...
                if save_spam("spotify", False):
                    ...
                    await lionxub.send_message(BOTLOG_CHATID, stringy)
            ...
            try:
                ...
                full = (await lionxub(GetFullUserRequest(lionxub.uid))).full_user
                bio = full.about
                ...
                if to_insert:
                    ...
                    if new_bio:
                        ...
                        if not new_bio == bio:
                            try:
                                await lionxub(UpdateProfileRequest(about=new_bio))
                    ...
                    if not new_bio:
                        if save_spam("telegram", True):
                            ...
                            await lionxub.send_message(BOTLOG_CHATID, to_send)
                # not to_insert means no playback
                    ...
                    # back to the original one
                        ...
                        await lionxub(UpdateProfileRequest(about=old_bio))
...
async def spotifybio(event):
    ...
    if SP_DATABASE.SPOTIFY_MODE:
        SP_DATABASE.SPOTIFY_MODE = False
        if USER_INITIAL_BIO:
            await lionxub(UpdateProfileRequest(about=USER_INITIAL_BIO["bio"]))
        ...
        USER_INITIAL_BIO["bio"] = (
            (await lionxub(GetFullUserRequest(lionxub.uid))).full_user
        ).about or ""
...
def file_check():
    ...
    if not os.path.exists(logo):
        urllib.request.urlretrieve(
            "https://github.com/TeamLionX/LionX-Resources/raw/master/Resources/Spotify/lionx.png",
            logo,
        )
    if not os.path.exists(font_mid):
        urllib.request.urlretrieve(
            "https://github.com/TeamLionX/LionX-Resources/blob/master/Resources/Spotify/GoogleSans-Medium.ttf?raw=true",
            font_mid,
        )
    if not os.path.exists(font_bold):
        urllib.request.urlretrieve(
            "https://github.com/TeamLionX/LionX-Resources/blob/master/Resources/Spotify/GoogleSans-Bold.ttf?raw=true",
            font_bold,
        )
...
async def make_thumb(url, client, song, artist, now, full):
    ...
    lionx = Image.open(logo, "r").resize((int(1024 / 5), int(1024 / 5)))
    thumbmask = Image.new("RGBA", (1024, 1024), 0)
    thumbmask.paste(background, (0, 0))
    thumbmask.paste(lionx, (-30, 840), mask=lionx)
    ...
    try:
        ...
        myphoto = urllib.request.urlretrieve(
            "https://github.com/TeamLionX/LionX-Resources/raw/master/Resources/Spotify/SwagLionX.jpg"
        )
...
async def spotify_now(event):
    ...
    lionxevent = await edit_or_reply(event, "🎶 `Fetching...`")
    oauth = {"Authorization": "Bearer " + SP_DATABASE.return_token()}
    r = requests.get(
        "https://api.spotify.com/v1/me/player/currently-playing", headers=oauth
    )
