...
async def parseqr(event):
    ...
    t_response, e_response = (await _lionxutils.runcmd(command_to_exec))[:2]
...
async def _(event):
    ...
    if not r.ok:
        return await lionxevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await lionxevent.edit(output_str, link_preview=True)
