...
async def _(event):
    ...
    chat = "@VS_Robot"
    lionevent = await edit_or_reply(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        ...
        if not input_str:
            return await edit_or_reply(lionevent, response4.text)
...
async def parseqr(event):
    ...
    lionevent = await edit_or_reply(event, "`Decoding....`")
    ...
    t_response, e_response = (await _lionutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await edit_or_reply(lionevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await edit_or_reply(lionevent, f"**Failed to Decode:**\n`{result}`")
...
async def _(event):
    ...
    try:
        ...
        return await lionevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(lionevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    lionevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await lionevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
