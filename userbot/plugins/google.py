# reverse search and google search  plugin for lionx
import io
import os
import re
import urllib
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await lionxevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
...
async def _(img):
    ...
    if photo:
        ...
        try:
            ...
            return await lionxevent.edit("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await lionxevent.edit("`Unable to perform reverse search.`")
