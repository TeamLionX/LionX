...
@lionub.lion_cmd(
    ...
    info={
        ...
        "examples": ["{tr}hbase en Lionuserbot", "{tr}hbase de Q2F0dXNlcmJvdA=="],
    },
)
    ...
    if event.pattern_match.group(1) == "en":
        if string:
            ...
            if mediatype is None:
                ...
                lionevent = await edit_or_reply(event, "`Encoding ...`")
                c_time = time.time()
        ...
        await edit_or_reply(
            lionevent, result, file_name="encodedfile.txt", caption="It's Encoded"
        )
    else:
        try:
            lething = str(
