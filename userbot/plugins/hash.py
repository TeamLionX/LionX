    ...
    if event.pattern_match.group(1) == "en":
        if string:
            ...
            if mediatype is None:
                ...
                lionxevent = await edit_or_reply(event, "`Encoding ...`")
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    reply,
                    Config.TMP_DOWNLOAD_DIRECTORY,
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, lionxevent, c_time, "trying to download")
                    ),
                )
                lionxevent = await edit_or_reply(event, "`Encoding ...`")
        ...
        await edit_or_reply(
            lionxevent, result, file_name="encodedfile.txt", caption="It's Encoded"
        )
    else:
        try:
            lething = str(
