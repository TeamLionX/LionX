...
async def _(event):
    ...
    if not replied_user:
        return
    lionevent = await edit_or_reply(event, "`Fetching userinfo wait....`")
...
""".format(
    ...
    await edit_or_reply(lionevent, caption)
...
async def who(event):
    ...
    if not replied_user:
        return
    lion = await edit_or_reply(event, "`Fetching userinfo wait....`")
    replied_user = await event.client(GetFullUserRequest(replied_user.id))
    try:
        photo, caption = await fetch_info(replied_user, event)
    except AttributeError:
        return await edit_or_reply(lion, "`Could not fetch info of that user.`")
