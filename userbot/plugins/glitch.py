...
async def glitch(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    lioninput = int(lioninput) if lioninput else 2
    glitch_file = await _liontools.media_to_pic(event, reply)
    ...
    if cmd:
        ...
        await event.client.send_file(event.chat_id, glitched, reply_to=lionid)
    else:
