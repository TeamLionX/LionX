...
from telethon.errors.rpcerrorlist import PeerFloodError, ChatWriteForbiddenError, ChatAdminRequiredError
from telethon import functions
...
async def scrapper(event):
    ...
    if event.is_private:
        message = await event.edit("Man, do you think I can't differentiate between a channel/group and a personal chat?")
    ...
    try:
        ...
        if max==0:
            message = await event.edit("Zero members?! If you aren't in mood of adding members then why even using me?")
    ...
    try:
        ...
        message = await event.edit(f"I don't think `{chat}` is a correct chat. Or maybe you are banned from it." )
        return await delete_in(message, 8)

    if event.chat_id == chat.id or str(event.chat_id) == ("-100"+f"{chat.id}"):
        message = await event.edit("You just simply tried to add members from the same group. Like WTF? I guess you need to be added in a mental hospital.")
        return await delete_in(message, 8)
    ...
    try:
        ...
        message = await event.edit("I am really sorry, you don't have sufficient administrative privilages required to do that.")
        return await delete_in(message, 5)
    ...
    if event.is_group:
        async for user in event.client.iter_participants(chat.id, limit=max, aggressive=True): # This limit thing isn't blowing anything...!
            if max == 0:
                return
            ...
            try:
                ...
                await event.delete()
                return await event.respond("You have insufficient rights to invite members here.")
            except PeerFloodError:
                if not warn_surpress:
                    warn_surpress = True
                    await event.respond("**I am really sorry. You have got Flood-Error from Telegram. Maybe you can't add non-mutual contacts. Please contact** @spambot **for more info.**")
            except Exception:
                failed+=1
    ...
    await event.delete()
    await event.respond(f"**Phew Done**\n__Stats:__\n**Kidnapped** __{success}__ **members. But couldn't kidnap** __{failed}__ **nerds.**")

    try:
        del Scrapping[event.chat_id]
    except KeyError:
        await event.respond("**Wait!! I just got some error. Please restart the bot quickly or reinstall the plugin!!**")
...
async def kill_scrapper(event):
    ...
    if event.chat_id not in Scrapping:
        ...
        try:
            ...
            return await event.edit("Got something wrong. Please reinstall the plugin or restart the bot.")
