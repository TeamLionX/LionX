...
async def download_audio(event):
    ...
    for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
        if _path.lower().endswith((".jpg", ".png", ".webp")):
            ...
            _fpath = _path
    if not _fpath:
        return await edit_delete(lionevent, "__Unable to upload file__")
...
async def download_video(event):
    ...
    lionevent = await edit_or_reply(event, "`Preparing to download...`")
    reply_to_id = await reply_id(event)
    ytdl_data = await ytdl_down(lionevent, video_opts, url)
    if ytdl_down is None:
        return
    f = pathlib.Path(f"{ytdl_data['title']}.mp4".replace("|", "_"))
    lionthumb = pathlib.Path(f"{ytdl_data['title']}.jpg".replace("|", "_"))
    if not os.path.exists(lionthumb):
        lionthumb = pathlib.Path(f"{ytdl_data['title']}.webp".replace("|", "_"))
    if not os.path.exists(lionthumb):
        lionthumb = None
    await lionevent.edit(
        f"`Preparing to upload video:`\
    ...
    uploaded = await event.client.fast_upload_file(
        file=ul,
        progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
            progress(d, t, lionevent, c_time, "upload", file_name=f)
        ),
    )
    ul.close()
    media = types.InputMediaUploadedDocument(
...
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        ...
        start = datetime.now()
        lionevent = await edit_or_reply(event, "**Downloading.....**")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await lionevent.edit("**Error:** `unblock` @instasavegrambot `and retry!`")
            return
        await lionevent.delete()
        lion = await event.client.send_file(
            event.chat_id,
            video,
        )
        end = datetime.now()
