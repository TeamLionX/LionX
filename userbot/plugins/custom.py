...
from userbot import BOTLOG_CHATID, lionxub
...
@lionxub.lionx_cmd(
    ...
    info={
        ...
        "var name": "**[list of vars]**(https://TeamLionX.gitbook.io/TeamLionX/data-vars-setup)",
        "usage": [
        ...
        if cmd == "set":
            if vname == "DEFAULT_USER":
                ...
                if FULL_USERINFO.about:
                    ...
                    delgvar("DEFAULT_BIO")
                try:
                    photos = await lionxub.get_profile_photos(lionxub.uid)
                    myphoto = await lionxub.download_media(photos[0])
                ...
                if not vinfo and vname == "ALIVE_TEMPLATE":
                    return await edit_delete(event, "Check @lionx_alive")
                if not vinfo:
                    return await edit_delete(
                        event,
                ...
                for i in check:
                    if vname == "DEFAULT_PIC" and not url(i):
                        ...
                        return await edit_delete(event, "**Give me a correct link...**")
                    elif (vname == "DIGITAL_PIC" or vname == "DEFAULT_PIC") and url(i):
                        vinfo = i
                        break
