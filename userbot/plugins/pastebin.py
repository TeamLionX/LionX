...
async def get_dogbin_content(event):
    ...
    try:
        ...
        return await lionxevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await lionxevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await lionxevent.edit(result, link_preview=True)
