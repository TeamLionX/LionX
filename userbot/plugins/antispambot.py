...
from . import BOTLOG, BOTLOG_CHATID, lionxub, edit_or_reply, logging, spamwatch
...
if Config.ANTISPAMBOT_BAN:
    ...
    async def anti_spambot(event):  # sourcery no-metrics
        ...
        lionxadmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not lionxadmin:
            return
        lionxbanned = None
        adder = None
